...
- **Phase 7 - Robust error handling + message history**: replaced `.unwrap()`s throughout the crate with `anyhow::Result` and `?` propagation, `main`, `FireKeeper`, `client::handle_client`, and `db.rs` all return `Result`s now, with per-client failures caught and logged inside the spawned task rather than crashing the server. Also added history replay: new clients receive the last 20 messages from SQLite (oldest-first) immediately after joining, via a new `FireKeeper::inform_context` step.
- **Later phases (planned)** - channels/rooms instead of global broadcast, basic auth, and eventually a real client with its own window (likely `egui`).
